...
parser = None
if perform_parsing:
    #grammar = nltk.data.load(path.join('grammars','large_grammars','alvey.fcfg'))
    grammar = nltk.data.load(path.join('grammars','large_grammars','atis.cfg'))
    #grammar = nltk.data.load(path.join('grammars','large_grammars','commandtalk.cfg'))
...
def process_text(text):
    ...
    num_sentences = 0
    num_vps = 0
    doc_sents[doc_id] = [] # store sentences per document
    print >> sys.stderr, 'sentences:'
    for s in sentences:
        print >> sys.stderr, s, '\n'
        if perform_parsing:
            # compute number verb phrases feature
            # tokenize the sentences to prep for POS tagging
            sent_tokens = nltk.word_tokenize(s)
            sent_tokens = stem_tokens_stopwords(sent_tokens, use_stops = False)[0]
            parse = None
            try:
                parse = parser.parse(sent_tokens)

                print >> sys.stderr, 'parse:\n'
                print >> sys.stderr, parse
            except Exception as e: # do nothing if grammar doesn't cover the input words
                if not 'Grammar does not cover some of the input words' in str(e):
                    raise
                else:
...
def print_stats(doc):
    ...
    wt_avg_sent_length_tfidf = calc_wt_avg_tfidf_text(' '.join(doc_sents[doc]), len(doc_sents[doc]))
